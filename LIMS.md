...
Leaning on the requirements of the previous application, I has able to create a process that would take user requests and scrum any associated run data for relevant headers based on equipment type. I personally created the middleware script using Python 3 and Illumina's InterOp library, allowing me to read the sequencer's binary files from the asscociate isilon spaces and extract meaningful metrics to be delivered back to a Salesforce LIMS application.
